...
public class GuiLexicon extends GuiScreen 
{
	...
	protected void drawTitle()
	{
		int i = (this.width - this.imageWidth) / 2;
		int j = (this.height - this.imageHeight) / 2;
		this.fontRendererObj.drawString("Lexicon", i + 15, j + 5, 0xffffff);
	}
	...
	protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_)
	{
		...
		for (int l3 = 0; (float)l3 * f - (float)j2 < 155.0F; ++l3)
		{
			...
			for (int i4 = 0; (float)i4 * f1 - (float)i2 < 224.0F; ++i4)
			{
				...
				this.mc.getTextureManager().bindTexture(ACHIEVEMENT_BACKGROUND1);
		...
		// This is reponsible for drawing arrow and displaying achievement 
